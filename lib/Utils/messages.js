...
async function processLocationThumbnail(input) {
    let buffer
...
const generateWAMessageContent = async (message, options) => {
    ...
    const thumb = await processLocationThumbnail(
        bl.thumb || bl.thumbnail
    )
    ...
    m = {
        buttonsMessage: {
            ...
            buttons: (bl.buttons || []).map(btn => ({
                ...
                buttonText: {
                    displayText: btn.displayText || btn.name || "Button"
                },
                type: 1
            })),
            headerType: 6
